    ...
    if args.load_model:
        ...
        if args.split_ratio:
            ...
            try:
                ...
                log.warning("Calendar split failed (%s) — falling back to 80/20 row split", e)
                split_idx = int(len(df_norm) * 0.8)
                baseline_df = df_norm.iloc[:split_idx].copy()
                score_df    = df_norm.iloc[split_idx:].copy()
                print(f"  {_yellow(f'Calendar split failed: {e}')}")
                print(f"  Baseline: {len(baseline_df):,} events (first 80%)")
                print(f"  Scoring:  {len(score_df):,} events (remaining 20%)")
