...
def build_tokens(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- Context flags ---
    out["has_encoded"]         = cmd.str.contains(ENCODED_RE, regex=True).fillna(False).astype(bool)
    out["has_download_cradle"] = cmd.str.contains(DOWNLOAD_CRADLE_RE, regex=True).fillna(False).astype(bool)
    out["has_bypass"]          = cmd.str.contains(BYPASS_RE, regex=True).fillna(False).astype(bool)
    out["has_reflection"]      = cmd.str.contains(REFLECTION_RE, regex=True).fillna(False).astype(bool)
