...
def build_cmdline_vectorizer(
    ...
    Returns None if the vocabulary is empty (e.g. all command lines are
    blank or missing), which causes the context channel to skip the
    cmdline novelty component gracefully.
    """
    # Filter to non-trivial command lines (not empty / whitespace-only)
    filled = baseline_commands.fillna("").astype(str)
    non_empty = filled[filled.str.strip().astype(bool)]
    if len(non_empty) < 3:
        logger.warning(
            "build_cmdline_vectorizer: fewer than 3 non-empty command lines "
            "(%d found) — skipping TF-IDF vectorizer", len(non_empty)
        )
        return None
    ...
    # min_df filtering can still produce an empty vocabulary
    if not vectorizer.vocabulary_:
        logger.warning(
            "build_cmdline_vectorizer: TF-IDF vocabulary is empty after "
            "min_df filtering — skipping cmdline novelty"
        )
        return None
...
def score_cmdline_novelty(
    ...
    """
    baseline_filled = baseline_commands.fillna("")
    test_filled = test_commands.fillna("")

    baseline_raw = vectorizer.transform(baseline_filled)
    test_raw = vectorizer.transform(test_filled)

    # Guard: if baseline or test matrix is entirely zero (all empty/OOV
    # commands), cosine similarity is undefined. Return 0 (neutral).
    if baseline_raw.nnz == 0 or test_raw.nnz == 0:
        return pd.Series(0.0, index=test_commands.index, name="cmdline_novelty")

    baseline_matrix = normalize(baseline_raw)
    test_matrix = normalize(test_raw)
    ...
    for i in range(0, test_matrix.shape[0], chunk_size):
        ...
        # Rows that were all-zero before normalize become NaN rows;
        # treat them as maximally novel (score = 1.0) or neutral (0.0).
        sim = np.nan_to_num(sim, nan=0.0)
        k = min(k_nearest, sim.shape[1])
